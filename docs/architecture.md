...
`scripts/package.ps1` 先发布自包含的 `win-x64` 应用，由 Velopack 生成内部安装引导程序、完整包和差分包，再由 Inno Setup 包装为面向用户的经典单 EXE 安装向导。应用从环境变量 `DOCVISTA_UPDATE_FEED` 或安装目录的 `update-feed.txt` 读取 HTTPS 更新源。正式发布必须向脚本传入代码签名参数。
