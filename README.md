...
```powershell
.\scripts\package.ps1 -Version 0.1.3 -UpdateFeedUrl https://updates.example.com/docvista/win
```

正式发布时通过 `-SignParams` 传入 `signtool` 参数。稳定通道使用 `win`，测试通道使用 `beta`。本地交付目录为 `artifacts\installer`，其中只保留 Setup EXE；CI 使用 `-KeepUpdateAssets` 暂存在线更新所需的后台文件。

Setup 安装后会创建桌面和开始菜单快捷方式。应用侧栏可启动官方卸载程序，卸载应用和快捷方式，不会改动用户文档。
